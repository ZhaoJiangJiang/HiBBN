...
def case_study_nyt(input_tokens, epoch_predicts, epoch_labels, vocab, cs_path, threshold=0.5, top_k=None):
    assert len(epoch_predicts) == len(epoch_labels), 'mismatch between prediction and ground truth for evaluation'
    assert len(input_tokens) == len(epoch_labels), 'mismatch between prediction and ground truth for evaluation'
    assert len(input_tokens) == len(epoch_predicts), 'mismatch between prediction and ground truth for evaluation'
    label2id = vocab.v2i['label']
    id2label = vocab.i2v['label']
    epoch_gold_label = list()
    # get id label name of ground truth
    ...
    for sample_predict in epoch_predicts:
        ...
        for j in range(top_k):
            if np_sample_predict[sample_predict_descent_idx[j]] > threshold:
                curr_predict.append(id2label[sample_predict_descent_idx[j]])
        epoch_predict_labels.append(curr_predict)

    output_data = pd.DataFrame(columns=['text', 'logits', 'labels'])

    for i in range(len(input_tokens)):
        output_data.loc[i] = [input_tokens[i], epoch_predict_labels[i], epoch_gold_label[i]]

    print(output_data.shape)
    output_data.to_csv(cs_path)

def evaluate_nyt_layer(epoch_predicts, epoch_labels, vocab, threshold=0.5, top_k=None):
    assert len(epoch_predicts) == len(epoch_labels), 'mismatch between prediction and ground truth for evaluation'
    label2id = vocab.v2i['label']
    id2label = vocab.i2v['label']
    epoch_gold_label = list()
    ...
    rcv1_layer1 = ['Top/News', 'Top/Classifieds', 'Top/Opinion', 'Top/Features']
    rcv1_layer2 = ['Top/News/Corrections', 'Top/Features/Dining and Wine', 'Top/News/U.S.', 'Top/News/Washington', 'Top/Features/Week in Review', 'Top/Features/Travel', 'Top/Features/Home and Garden', 'Top/News/Education', 'Top/Features/Books', 'Top/Classifieds/Paid Death Notices', 'Top/News/Front Page', 'Top/News/Obituaries', 'Top/News/Sports', 'Top/News/Health', 'Top/Features/Magazine', 'Top/Features/Movies', 'Top/Features/Theater', 'Top/Classifieds/Job Market', 'Top/News/Science', 'Top/News/World', 'Top/Features/Style', 'Top/News/Technology', 'Top/Features/Arts', 'Top/News/Business', 'Top/News/New York and Region', 'Top/Classifieds/Real Estate', 'Top/Opinion/Opinion']
    rcv1_layer3 = ['Top/Features/Theater/Plays', 'Top/Opinion/Opinion/Editorials', 'Top/Classifieds/Real Estate/Locations', 'Top/News/Business/Your Money', 'Top/Features/Movies/News and Features', 'Top/News/Sports/Baseball', 'Top/Opinion/Opinion/Letters', 'Top/News/Sports/Columns', 'Top/Features/Style/Fashion and Style', 'Top/News/Business/Columns', 'Top/News/U.S./Midwest', 'Top/News/New York and Region/The City', 'Top/Features/Theater/Reviews', 'Top/News/U.S./West', 'Top/News/Business/Markets', 'Top/News/Science/Environment', 'Top/News/Business/Small Business', 'Top/News/Washington/Campaign 2008', 'Top/Features/Movies/Reviews', 'Top/News/New York and Region/Long Island', 'Top/News/New York and Region/New Jersey', 'Top/News/Business/Media and Advertising', 'Top/News/Sports/Pro Football', 'Top/News/World/Europe', 'Top/News/World/Middle East', 'Top/Features/Arts/Dance', 'Top/Opinion/Opinion/Op-Ed', 'Top/Features/Travel/Sunday Travel', 'Top/News/World/Africa', 'Top/Classifieds/Job Market/Job Categories', 'Top/News/U.S./New England', 'Top/News/U.S./South', 'Top/News/World/Asia Pacific', 'Top/News/U.S./Mid-Atlantic', 'Top/Features/Arts/Music', 'Top/News/Sports/Hockey', 'Top/News/New York and Region/Connecticut', 'Top/Features/Travel/Guides', 'Top/News/World/Americas', 'Top/News/New York and Region/Columns', 'Top/News/U.S./Southwest', 'Top/News/World/Countries and Territories', 'Top/News/Health/Diseases, Conditions, and Health Topics', 'Top/Features/Arts/Art and Design', 'Top/News/Sports/Pro Basketball', 'Top/News/Technology/Circuits', 'Top/News/New York and Region/Westchester', 'Top/News/Washington/Campaign 2004', 'Top/Features/Books/Book Reviews', 'Top/Features/Theater/News and Features', 'Top/News/U.S./U.S. States, Territories and Possessions']
